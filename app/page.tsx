...
import { AlertTriangle, Brain, BriefcaseBusiness, CheckCircle2, Cloud, Download, Focus, GitBranch, LayoutDashboard, Lightbulb, ListChecks, LockKeyhole, LogOut, Maximize2, MemoryStick, Plus, Search, Sparkles, Target, X } from "lucide-react";
...
const colors: Record<Kind,string> = { company:"#8ab4ff",decision:"#66d6bd",memory:"#c7d8e7",risk:"#ff8f86",task:"#f4c76f",pattern:"#b79cff",goal:"#b9df80" };
...
export default function Home() {
  ...
  async function unlock() {
    ...
    setStore(result.data); setSha(result.sha); setUnlocked(true); setStatus(`Private memory synced • ${result.data.nodes.length} records`);
  }

  function lock() { setStore(emptyStore); setSha(""); setAccessKey(""); setSelected(null); setWorkspace(null); setFocusNode(null); setUnlocked(false); setStatus("Locked"); }
  ...
  async function persist(next: Store) {
    ...
    const result = await response.json();
    if (response.ok) { setSha(result.sha); setStatus("Saved to private GitHub memory"); } else setStatus(result.error || "Sync failed");
    setSaving(false);
  }
  ...
  async function add() {
    ...
    const next = { ...store, nodes: [node, ...store.nodes], links: [...refs.map((r) => ({ source: id, target: r.id, type: "belongs_to" })), ...store.links] };
  ...
  const projects = byKind("company"), decisions = byKind("decision"), risks = byKind("risk"), tasks = byKind("task"), patterns = byKind("pattern");
  ...
  const focusIds = useMemo(() => {
    ...
    return new Set([focusNode, ...related(focusNode).map(n => n.id)]);
  }, [focusNode, store.links, store.nodes]);

  const visible = useMemo(() => store.nodes.filter((n) => {
    const textMatch = `${n.title} ${n.body} ${n.kind}`.toLowerCase().includes(q.toLowerCase());
    const kindMatch = kindFilter === "all" || n.kind === kindFilter;
    return textMatch && kindMatch && (!focusIds || focusIds.has(n.id));
  }), [store.nodes, q, kindFilter, focusIds]);

  const degree = useMemo(() => {
    const map: Record<string,number> = {};
    store.links.forEach(l => { map[l.source] = (map[l.source] || 0) + 1; map[l.target] = (map[l.target] || 0) + 1; });
    return map;
  }, [store.links]);
  ...
  function backup() { const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([JSON.stringify(store, null, 2)], { type: "application/json" })); a.download = "alos-backup.json"; a.click(); }
  function fitGraph() { setTimeout(() => graphRef.current?.zoomToFit?.(500, 70), 50); }
  function clearFocus() { setFocusNode(null); setSelected(null); setTimeout(() => fitGraph(), 80); }
  ...
  return <main>
    <aside className="sidebar">
      ...
      <button onClick={() => { setTab("graph"); setWorkspace(null); setTimeout(fitGraph,100); }} className={tab === "graph" ? "active" : ""}><GitBranch/>Graph</button>
    ...
    <section className="work">
      ...
      {workspace ? <section className="workspace"><button className="back-link" onClick={()=>setWorkspace(null)}>← Back to projects</button><div className="workspace-hero"><div><span className="pill">project workspace</span><h2>{workspace.title}</h2><p>{workspace.body}</p></div><div className="workspace-score"><b>{projectRelated.length}</b><span>connected records</span></div></div><div className="workspace-grid">{(["task","risk","decision","goal","pattern","memory"] as Kind[]).map(kind=><section className="panel" key={kind}><div className="panel-title"><i className={kind}/>{kind}s</div>{projectRelated.filter(n=>n.kind===kind).length ? projectRelated.filter(n=>n.kind===kind).map(n=><button className="row" key={n.id} onClick={()=>setSelected(n)}><span>{n.title}</span><small>{kind}</small></button>) : <p className="empty">No connected {kind}s yet.</p>}</section>)}</div></section> : <>
        {tab === "home" && <><section className="brief"><div><span><Sparkles/>CEO daily brief</span><h2>{action}</h2><p>Finish one high-value outcome before switching projects. Resolve the most consequential blocker first.</p></div><div className="score"><b>{executionScore}</b><small>execution score</small></div></section><div className="metric-grid"><article><b>{projects.length}</b><span>Projects</span></article><article><b>{tasks.length}</b><span>Open tasks</span></article><article><b>{risks.length}</b><span>Risks</span></article><article><b>{decisions.length}</b><span>Decisions</span></article></div><div className="dashboard-grid"><section className="panel"><div className="panel-title"><Target/>Today&apos;s focus</div>{tasks.slice(0,4).map(n=><button className="row" key={n.id} onClick={()=>setSelected(n)}><span>{n.title}</span><small>task</small></button>)}</section><section className="panel"><div className="panel-title"><AlertTriangle/>Current blockers</div>{risks.slice(0,4).map(n=><button className="row" key={n.id} onClick={()=>setSelected(n)}><span>{n.title}</span><small>risk</small></button>)}</section><section className="panel"><div className="panel-title"><Lightbulb/>Patterns noticed</div>{patterns.slice(0,4).map(n=><button className="row" key={n.id} onClick={()=>setSelected(n)}><span>{n.title}</span><small>pattern</small></button>)}</section><section className="panel"><div className="panel-title"><ListChecks/>Recent decisions</div>{decisions.slice(0,4).map(n=><button className="row" key={n.id} onClick={()=>setSelected(n)}><span>{n.title}</span><small>decision</small></button>)}</section></div></>}
        ...
        {tab === "graph" && <section className="obsidian-wrap"><div className="graph-topbar"><div className="legend">{kinds.map(k=><button key={k} className={kindFilter===k?"selected":""} onClick={()=>setKindFilter(kindFilter===k?"all":k)}><i className={k}/>{k}</button>)}</div><div className="graph-actions"><button onClick={fitGraph}><Maximize2/>Fit</button>{focusNode && <button onClick={clearFocus}><X/>Clear focus</button>}</div></div><div className="obsidian-shell"><div className="graph"><Graph ref={graphRef} graphData={graphData} backgroundColor="#0c0f14" cooldownTicks={160} d3AlphaDecay={0.025} d3VelocityDecay={0.32} warmupTicks={80} nodeRelSize={5} nodeLabel={(n:any)=>`${n.title}\n${n.kind}`} linkColor={()=>"rgba(157,170,195,.24)"} linkWidth={(l:any)=>focusNode && (l.source?.id===focusNode||l.target?.id===focusNode||l.source===focusNode||l.target===focusNode) ? 1.6 : .55} onNodeClick={(n:any)=>{setSelected(n);setFocusNode(n.id)}} onBackgroundClick={()=>setSelected(null)} nodeCanvasObjectMode={()=>"replace"} nodeCanvasObject={(node:any,ctx:any,scale:number)=>{const active=selected?.id===node.id||focusNode===node.id;const r=Math.min(10,4.5+(degree[node.id]||0)*.55);ctx.beginPath();ctx.arc(node.x,node.y,active?r+2:r,0,Math.PI*2);ctx.fillStyle=colors[node.kind as Kind];ctx.shadowColor=active?colors[node.kind as Kind]:"transparent";ctx.shadowBlur=active?16:0;ctx.fill();ctx.shadowBlur=0;const show=scale>1.35||active||node.kind==="company";if(show){const label=node.title.length>34?node.title.slice(0,31)+"...":node.title;ctx.font=`${active?600:500} ${Math.max(9,11/scale)}px Inter, sans-serif`;ctx.fillStyle=active?"#ffffff":"#d5dbea";ctx.textAlign="center";ctx.fillText(label,node.x,node.y+r+12/scale)}}}/></div><aside className="graph-panel"><h3>{selected ? selected.title : "Knowledge graph"}</h3>{selected ? <><span className="pill">{selected.kind}</span><p>{selected.body}</p><button className="focus-btn" onClick={()=>setFocusNode(selected.id)}><Focus/>Focus connections</button><h4>Connected</h4><div className="mini-list">{related(selected.id).slice(0,10).map(n=><button key={n.id} onClick={()=>{setSelected(n);setFocusNode(n.id)}}><i className={n.kind}/><span>{n.title}</span></button>)}</div></> : <><p>Like Obsidian, larger nodes have more connections. Zoom in to reveal labels. Tap a node to inspect it, then focus to isolate its neighborhood.</p><div className="graph-stats"><span><b>{graphData.nodes.length}</b> nodes</span><span><b>{graphData.links.length}</b> links</span></div></>}</aside></div></section>}
      </>}
    </section>

    {selected && tab !== "graph" && <div className="shade" onClick={()=>setSelected(null)}><div className="drawer" onClick={e=>e.stopPropagation()}><button className="x" onClick={()=>setSelected(null)}>×</button><span className="pill">{selected.kind}</span><h2>{selected.title}</h2><p>{selected.body}</p><hr/><h3>Connected to</h3><div className="related">{related(selected.id).slice(0,12).map(n=><button key={n.id} onClick={()=>setSelected(n)}>{n.title}<small>{n.kind}</small></button>)}</div><hr/><h3>Was this useful?</h3><div className="feedback"><button onClick={()=>addFeedback(selected.id,true)}><CheckCircle2/>Useful</button><button onClick={()=>addFeedback(selected.id,false)}><AlertTriangle/>Not useful</button></div></div></div>}
  </main>;
}
