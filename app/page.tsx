...
export default function Home() {
  ...
  return <main>
    <aside>
      ...
      <button onClick={() => setTab("home")} className={tab === "home" ? "active" : ""}><LayoutDashboard/>Home</button>
    ...
    <section className="work">
      ...
      {tab === "home" && <>
        ...
        <div className="dashboard-grid">
          ...
          <section className="panel"><div className="panel-title"><Lightbulb/>Patterns noticed</div>{patterns.slice(0,4).map(n=><button className="row" key={n.id} onClick={()=>setSelected(n)}><span>{n.title}</span><small>pattern</small></button>)}</section>
          <section className="panel"><div className="panel-title"><ListChecks/>Recent decisions</div>{decisions.slice(0,4).map(n=><button className="row" key={n.id} onClick={()=>setSelected(n)}><span>{n.title}</span><small>decision</small></button>)}</section>
        </div>
      </>}
